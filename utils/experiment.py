...
def train(model, train_loader, optimizer, loss_fn, mask):
    ...
    for batch_idx, (text, target) in enumerate(train_loader):
        ...
        all_corr = compute_corr_coeff(output_np.reshape(output_np.shape[0], -1), target_np.reshape(output_np.shape[0], -1))
        corr = np.mean(np.diag(all_corr))
        if np.isnan(corr):
            print(text, np.isnan(output_np).any(), np.isnan(target_np).any(), all_corr, corr)
            print("Output", torch.max(output), output)
            print("Target", torch.max(target), target)
        #     print("Masked output", np.max(output_np))
        #     print("Masked target", np.max(target_np))
            sys.exit(1)
...
def eval(model, val_loader, loss_fn, mask):
    ...
    with torch.no_grad():
        for batch_idx, (text, target) in enumerate(val_loader):
            ...
            output_np = output.cpu().detach().numpy()[:, :, mask]
            target_np = target.cpu().detach().numpy()[:, :, mask]
            corr = np.mean(np.diag(compute_corr_coeff(output_np.reshape(output_np.shape[0], -1), target_np.reshape(output_np.shape[0], -1))))
            avg_corr = avg_corr + corr / len(val_loader)

    print('  Val: avg loss: {:.6f} - avg corr: {:.6f}'.format(avg_loss, avg_corr))
